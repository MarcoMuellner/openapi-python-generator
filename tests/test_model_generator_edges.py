...
def test_type_converter_nested_allof_oneof_anyof():
    ...
    inner_oneof = Schema(oneOf=[Schema(type=DataType.STRING), Schema(type=DataType.INTEGER)])
    inner_anyof = Schema(anyOf=[Schema(type=DataType.BOOLEAN), Schema(type=DataType.NUMBER)])
    ...
    assert "Union[str,int]" in tc.converted_type or "Union[int,str]" in tc.converted_type
